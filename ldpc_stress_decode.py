...
def print_report(stats: dict, wav_path: str, csv_path: str,
    ...
    line = "═" * 62
    print(line)
    print("  GUST LDPC Stresstest-Auswertung")
    print(line)
...
def main() -> int:
    ...
    # LDPC-Backend aktivieren (vor dem Scan, Etappe 4)
    import gust_ldpc
    from gust_frame import set_fec_backend
    set_fec_backend("ldpc")

    # Warnung wenn Dateiname kein "ldpc_" enthält (RS-WAV → falsches Backend)
    if "ldpc_" not in os.path.basename(args.wav):
